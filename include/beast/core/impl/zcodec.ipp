...
#ifndef BEAST_IMPL_ZCODEC_IPP
#define BEAST_IMPL_ZCODEC_IPP
...
void
z_istream_t<_>::window::
read(std::uint8_t* out, std::uint16_t n, std::uint16_t pos)
{
    // VFALCO This could be optimized by repeating
    // memcpy instead of working a byte at a time.
    // Does this function get called with large values of n?
    //
    std::uint16_t i = (i_ - pos + size_) % size_;
    for(n--)
    {
        auto const v = p_[i];
        p_[i_] = *out++ = v;
    ...
    ConstBufferSequence const& in, error_code& ec)
{
    ...
    for(;;)
    {
        ...
        dynabuf.commit(m);
//std::string s = to_string(dynabuf.data());
//std::cerr << s;
        used += n;
        if(ec)
            break;
...
write(params& ps, error_code& ec)
{
    ...
    auto have = ps.avail_in;
    auto const outend = ps.next_out + ps.avail_out;
    auto const end = next + have;
    auto const err =
        [&](zcodec_error ev)
        {
            ec = ev;
    ...
    for(;;)
    {
        ...
        case s_typedo:
        {
            if(! bi_.fill(3, next, end))
                return more();
            std::uint8_t v;
            bi_.read(1, v, next, end);
            last_ = v != 0;
            bi_.read(2, v, next, end);
            switch(v)
            {
            case 0: // uncompressed
                s_ = s_stored;
            ...
            default:
                return err(zcodec_error::invalid_block_type);
            }
            break;
        }
        ...
        case s_table:
            if(! bi_.fill(5 + 5 + 4, next, end))
                return more();
            bi_.read(5, nlen_, next, end);
            nlen_ += 257;
            bi_.read(5, ndist_, next, end);
            ndist_ += 1;
            bi_.read(4, ncode_, next, end);
            ncode_ += 4;
            i_ = 0;
            s_ = s_lenlens;
            // fall through
        ...
        case s_lenlens:
        {
            ...
            for(;;)
            {
                if(i_ < ncode_ - 4)
                {
                    if(! bi_.read(3, lens_[si[i_]], next, end))
                        return more();
                    ++i_;
                }
                else
            ...
            next_ = codes_.data();
            auto const result =
                mktab(CODES, &lens_[0], 19, &next_, &lenbits_, work_);
            s_ = s_codelens;
            // fall through
        }
        ...
        case s_len:
        {
            std::uint16_t v;
            back_ = 0;
            if(! bi_.peek(lenbits_, v, next, end))
                return more();
            auto cp = &lencode_[v];
            if(cp->op && (cp->op & 0xf0) == 0)
            {
                auto prev = cp;
                if(! bi_.peek(prev->bits + prev->op, v, next, end))
                    return more();
            ...
            if(cp->op & 64)
                return err(zcodec_error::invalid_len_code);
            extra_ = cp->op & 15;
            s_ = s_lenext;
            // fall through
        }
        ...
        {
            std::uint16_t v;
            if(! bi_.peek(distbits_, v, next, end))
                return more();
            auto cp = &distcode_[v];
            if((cp->op & 0xf0) == 0)
            {
            ...
            if(cp->op & 64)
                return err(zcodec_error::invalid_dist_code);
            offset_ = cp->val;
            extra_ = cp->op & 15;
            s_ = s_distext;
            // fall through
